...
export function layoutsReducer(state: LayoutState,
  ...
  if (action.type === getType(layoutActions.baseAddGrouptoLayout)) {
    ...
    if (action.payload.dir == 'top' || action.payload.dir == 'bottom' ) {
      ...
      const signalName = propSg(Number(id), "layout", "row_" + action.payload.newDim+"_size");
      ...
      const signalName = propSg(Number(id), "layout", "col_" + action.payload.newDim+"_size");
      ...
      state = state.setIn([id, "colSizes"], [{signal: propSg(Number(id), "layout", "col_" + action.payload.newDim+"_size")}]);
      state = state.setIn([id, "rowSizes"], [{signal: propSg(Number(id), "layout", "row_" + action.payload.newDim+"_size")}]);
