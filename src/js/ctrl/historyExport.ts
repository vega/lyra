 ...
 */
export function historyExporter(internal: boolean = false, history): Spec {
  if (history.getIn(['marks']).size == 0) {
    return {};
  }
  ...
  let spec: Spec = historyExporter.scene(state, history, int);
  spec.data = historyExporter.pipelines(state, history, int);

  // Add interactions and widgets from store
  spec = historyExporter.interactions(state, history, spec);
  spec = historyExporter.widgets(state, history, spec);
  return spec;
}

historyExporter.interactions = function(state: State, history, spec) {
  history.getIn(['interactions']).forEach((interaction: InteractionRecord) => {
    const group: GroupRecord = history.getIn(['marks', String(interaction.groupId)]);
    ...
    const mouseTypes = group._interactions.map(interactionId => {
      const interaction: InteractionRecord = history.getIn(['interactions', String(interactionId)]);
...
historyExporter.widgets = function(state: State, history, spec) {
  history.getIn(['widgets']).forEach((widget: WidgetRecord) => {
    const group: GroupRecord = history.getIn(['marks', String(widget.groupId)]);
    const groupName = exportName(group.name);
    if (widget.selection) {
...
historyExporter.pipelines = function(state: State, history, internal: boolean) {
  const pipelines: PipelineRecord[] = history.getIn(["pipelines"]).valueSeq();
  return pipelines.reduce(function(spec, pipeline) {
    spec.push(historyExporter.dataset(state, history, internal, pipeline._source));

    const aggrs = pipeline._aggregates.toJS();
    for (const key in aggrs) {
      spec.push(historyExporter.dataset(state, history, internal, aggrs[key]));
    }
    return spec;
  }, []);
};

historyExporter.dataset = function(state: State, history, internal: boolean, id: number) {
  const dataset = history.getIn(['datasets', id.toString()]).toJS(),
    spec = clean(duplicate(dataset), internal),
    values = input(id),
    format = spec.format && spec.format.type,
    sort = historyExporter.sort(dataset);
  ...
  if (spec.source) {
    spec.source = name(history.getIn(['datasets' , spec.source , 'name']));
  } else if (internal) {
    spec.values = values;
    delete spec.url;
  ...
  const interactions: InteractionRecord[] = history.getIn(['interactions']).valueSeq().toArray();
  ...
  const widgets: WidgetRecord[] = history.getIn(['widgets']).valueSeq().toArray();
  ...
  spec.transform.forEach(s => {
    if (s.type === 'lookup') {
      s.from = history.getIn(['datasets', s.from, 'name'])
    }
    if (s.type === 'filter') {
 ...
 */
historyExporter.sort = function(dataset) {
...
historyExporter.scene = function(state: State, history, internal: boolean): Mark {
  const sceneId = history.getIn(['scene', '_id']);
  let spec = historyExporter.group(state, history, internal, sceneId);
...
historyExporter.mark = function(state: State, history, internal: boolean, id: number) {
  const mark = history.getIn(['marks' , id.toString()]).toJS();
  ...
  if (facet) {
    ...
    if ((fromId = spec.from.data)) {
      spec.from.data = name(history.getIn(['datasets', fromId.toString() , 'name']));

      const count = counts.data[fromId] || (counts.data[fromId] = duplicate(DATA_COUNT));
      count.marks[id] = true;
    } else if ((fromId = spec.from.mark)) {
      spec.from.mark = name(history.getIn(['marks' , fromId.toString() , 'name']));
    }
  }
  ...
  Object.keys(upspec).forEach(function(key) {
    ...
    if (origScale) {
      specVal.scale = name(history.getIn(['scales' , origScale.toString() , 'name']));
  ...
  if (internal) {
    ...
    return facet ? pathgroup(state, history, s, facet) : s;
  }

  return facet ? pathgroup(state, history, spec, facet) : spec;
};

function pathgroup(state, history, marks, facet) {
  return {
    name: 'pathgroup',
    type: 'group',
    from: {
      facet: {
        ...facet,
        data: name(history.getIn(['datasets' , facet.data , 'name']))
      }
    },
    encode: {
      update: {
...
historyExporter.group = function(state: State, history, internal: boolean, id: number) {
  const mark: GroupRecord = history.getIn(['marks', id.toString()]);
  const spec = historyExporter.mark(state, history, internal, id);
  ...
  ['scale', 'mark', 'axe', 'legend'].forEach(function(childType) {
    ...
    group[childTypes] = mark[childTypes]
      .map(cid => {
        const child = history.getIn([storePath,cid.toString()]);
        return !child ? null :
          historyExporter[child.type] ? historyExporter[child.type](state, history, internal, cid) :
          historyExporter[childType]  ? historyExporter[childType](state, history, internal, cid) :
...
historyExporter.area = function(state: State, history, internal: boolean, id: number) {
  const spec = historyExporter.mark(state, history, internal, id);
...
historyExporter.line = function(state: State, history, internal: boolean, id: number) {
  const spec = historyExporter.mark(state, history, internal, id);
...
historyExporter.scale = function(state: State, history, internal: boolean, id: number) {
  const scale = history.getIn(['scales', id.toString()]).toJS(),
  ...
  if (!scale.domain && scale._domain && scale._domain.length ) {
    spec.domain = scale._manual && scale._manualDomain.length ? scale._manualDomain : dataRef(state, history, scale, scale._domain);
  }

  if (!scale.range && scale._range && scale._range.length) {
    spec.range = dataRef(state, history, scale, scale._range);
  }
...
historyExporter.axe = historyExporter.legend = function(state: State, history, internal: boolean, id: number) {
  const guide = history.getIn(['guides', id.toString()]).toJS(),
  ...
  if (gtype === GuideType.Axis) {
    ...
    spec.scale = name(history.getIn(['scales' , spec.scale , 'name']));

  } else if (gtype === GuideType.Legend) {
    counts.scales[spec[type]].guides[id] = true;
    spec[type] = name(history.getIn(['scales' , spec[type] , 'name']));
  }
 ...
 */
function dataRef(state: State, history, scale, ref) {
  let sets = {},
    data,
    did,
    field,
  ...
  if (ref.length === 1) {
    ref = ref[0];
    data = history.getIn(['datasets' , ref.data.toString()]);
    return sortDataRef(data, scale, ref.field);
  }

  // More than one ref
  for (i = 0, len = ref.length; i < len; ++i) {
    data = history.getIn(['datasets' , ref[i].data.toString()]);
    field = ref[i].field;
  ...
  for (i = 0, len = keys.length; i < len; ++i) {
    data = history.getIn(['datasets' , keys[i]]);
